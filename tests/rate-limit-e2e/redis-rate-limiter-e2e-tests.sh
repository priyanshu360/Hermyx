...
TEST_RESULTS_FILE="comprehensive-redis-test-results.txt"
...
start_hermyx_redis() {
    ...
    wait_for_service "$HERMYX_URL/health" "Hermyx Redis"
}
...
test_redis_connectivity() {
    ...
    if docker exec hermyx-redis redis-cli ping > /dev/null 2>&1; then
        print_status "PASS" "Redis is accessible and responding"
    else
        print_status "FAIL" "Redis is not accessible"
        return 1
    fi
}
...
test_route_isolation() {
    ...
    local status_route2=$(make_request "$HERMYX_URL/json" "GET")
...
# Test: Redis stress test
test_redis_stress() {
    print_status "TEST" "Redis Stress - High volume request test"
    ...
    if [ $success_rate -ge 10 ] && [ $success_rate -le 90 ]; then
        print_status "PASS" "Redis Stress: System stable under load ($success_count/$total_requests succeeded)"
    else
        print_status "FAIL" "Redis Stress: Unexpected behavior ($success_count/$total_requests succeeded)"
    fi
}

# Test: Redis persistence verification
test_redis_persistence() {
    print_status "TEST" "Redis Persistence - Verify rate limits persist across restarts"
    
    # Make some requests to trigger rate limiting
    local success_count=0
    for ((i=1; i<=5; i++)); do
        local status_code
        status_code=$(make_request "$HERMYX_URL/get" "GET")
    ...
    # Restart Hermyx service
    print_status "INFO" "Restarting Hermyx Redis service..."
    docker-compose restart hermyx-redis
    
    # Wait for service to be ready
    wait_for_service "$HERMYX_URL/health" "Hermyx Redis (after restart)"
    
    # Make more requests - should still be rate limited
    local rate_limited_after_restart=0
    for ((i=1; i<=3; i++)); do
        local status_code
        status_code=$(make_request "$HERMYX_URL/get" "GET")
        
        if [ "$status_code" = "429" ]; then
            ((rate_limited_after_restart++))
        fi
        sleep 0.1
    done
    
    if [ $rate_limited_after_restart -gt 0 ]; then
        print_status "PASS" "Redis Persistence: Rate limits persisted across restart"
    else
        print_status "FAIL" "Redis Persistence: Rate limits did not persist across restart"
    fi
}

# Test: Redis state verification (persistent)
test_redis_state_persistence() {
    print_status "TEST" "Redis State - Verify rate limits ARE persistent (Redis-backed)"
    ...
    if [ "$status_blocked" = "429" ]; then
        ...
        print_status "PASS" "Redis State: Redis-backed rate limiting active (state persists across restarts)"
    else
        print_status "FAIL" "Redis State: Could not trigger rate limit"
    fi
}
...
main() {
    echo "=========================================================="
    echo "  COMPREHENSIVE REDIS RATE LIMITER E2E TEST SUITE"
    echo "=========================================================="
    echo "  Combining Original + Enhanced Test Coverage"
    echo "=========================================================="
    
    # Initialize test results file
    echo "Comprehensive Redis Rate Limiter E2E Test Results - $(date)" > "$TEST_RESULTS_FILE"
    ...
    # Start Hermyx with redis config
    start_hermyx_redis
    ...
    # Test 15: Redis Stress Test
    test_redis_stress
    sleep 2
    
    # ========================================================================
    # SECTION 10: REDIS-SPECIFIC CHARACTERISTICS
    # ========================================================================
    print_section "SECTION 10: REDIS-SPECIFIC CHARACTERISTICS"
    
    # Test 16: Redis State (Persistence)
    test_redis_state_persistence
    sleep 1
    
    # Test 17: Redis Persistence
    test_redis_persistence
    ...
    echo ""
    echo "=========================================================="
    echo "  COMPREHENSIVE REDIS RATE LIMITER TEST RESULTS"
    echo "=========================================================="
    ...
    print_status "INFO" "Hermyx logs saved to: $SCRIPT_DIR/logs/hermyx-redis.log"
    ...
    if [ $FAILED_TESTS -eq 0 ]; then
        print_status "PASS" "All Comprehensive Redis Rate Limiter tests passed!"
        exit 0
    else
        print_status "FAIL" "Some Comprehensive Redis Rate Limiter tests failed!"
        exit 1
    fi
}
