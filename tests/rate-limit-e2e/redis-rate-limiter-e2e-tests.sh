...
# This script tests various rate limiting scenarios using Redis storage
...
TEST_RESULTS_FILE="redis-test-results.txt"
...
test_redis_persistence() {
    print_status "INFO" "Testing Redis persistence - Rate limits should persist across restarts"
...
main() {
    echo "=========================================="
    echo "Redis Rate Limiter E2E Tests"
    echo "=========================================="
    
    # Initialize test results file
    echo "Redis Rate Limiter E2E Test Results - $(date)" > "$TEST_RESULTS_FILE"
    ...
    print_status "INFO" "Starting Redis Rate Limiter E2E Tests..."
    ...
    test_rate_limit "/uuid" 10 60 "Global API Rate Limiting"
    
    # Test 7: No Rate Limit API (should not be rate limited)
    test_no_rate_limit "/status/200" "No Rate Limiting"
    
    # Test 8: Redis Persistence
    test_redis_persistence
    
    # Print final results
    echo ""
    echo "=========================================="
    echo "Redis Rate Limiter E2E Test Results"
    echo "=========================================="
    ...
    if [ $FAILED_TESTS -eq 0 ]; then
        print_status "PASS" "All Redis Rate Limiter E2E tests passed!"
        exit 0
    else
        print_status "FAIL" "Some Redis Rate Limiter E2E tests failed!"
        exit 1
    fi
}
